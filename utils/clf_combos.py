...
def classify_combos(params):
    '''
    This function mades a classification in combos

    params (tuple):
    - amount of individual metrics
    - tuple with the df.keys() in combinations
    - label with y values
    - dataframe
    - classifier option
    - n_folds
    ...
    if parallelism_opt:
        ...
        if classifier_opt == 'qda':
            ...
            with Pool(processes=n_cpus) as pool:
                result = pool.map(classify_lda_cv_parallel.get_classifier, params, chunksize=batches)
                output = [x for x in result]
